...
> 生成时间：2026-06-30T15:40:39
> 数据来源：`pt9l_full_audit_output_langgraph_v6`。
> 口径：只把机械复审 confirm cluster 与语义 Challenge keep/revise 计入“当前确认错误”。`merged_findings` 是合并行数，不等于确认错误数。
...
- 机械/数值待复审候选：31 类，对应候选行 31 条（未计入确认错误）
- 语义确认/修订错误：45 条（keep 27，revise 18）
- 语义仍需人工补证疑点：21 条；已丢弃语义误报：1 条
- Challenge 理由质量保守降级：5 条
- 合并 findings 行数：223

公式：`当前确认错误总数 = 机械 confirm cluster 数 + 语义 keep/revise 数`。
...
## 6. 机械/数值待复审候选（未计入确认错误）

> 这些条目是机械复审中仍为 `needs_more_context` 的 cluster，当前不算错误。v6 中主要是新增 Parameter Alignment 数值候选，因为本轮不能外发新增证据到 LLM，所以保守保留给人工或后续内网模型复审。

### PM001. Parameter alignment candidate: operating_environment has multiple numeric values (16.0±2.0℃, 21±3℃, 23.0±2.0℃, 25±5℃, +15 °C, +35 °C, +40 °C, +5 °C) across controlled documents.

- Cluster：`MC-CLUSTER-0073`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 12 / `parameter_alignment:operating_environment:temperature_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `12 设计输出（含01、02）\设计输出清单二\装配说明V1.0-PT9L.via_xlsx.clean.md:30` operating_environment/temperature_value/temperature: 16.0±2.0℃, 21±3℃, 23.0±2.0℃, 25±5℃, +15 °C, +35 °C, +40 °C, +5 °C; stages: External Test, Stage 01, Stage 03, Stage 04, Stage 11, Stage 12, Stage 18, 历史记录

### PM002. Parameter alignment candidate: measurement_range has multiple numeric values (+15 °C, +35 °C, +40 °C, +5 °C, -104℉, -107.6℉, -109.2℉, -109.22℉) across controlled documents.

- Cluster：`MC-CLUSTER-0074`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_range:temperature_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-03 PT9L -2-56 报告-正式.pdf.md:477` measurement_range/temperature_value/temperature: +15 °C, +35 °C, +40 °C, +5 °C, -104℉, -107.6℉, -109.2℉, -109.22℉; stages: External Test, Stage 11, Stage 12, Stage 18, 历史记录

### PM003. Parameter alignment candidate: measurement_accuracy has multiple numeric values (16.0±2.0℃, 23.0±2.0℃, -0.1℃, -0.2℃, -104℉, -107.6℉, -109.2℉, -40℃) across controlled documents.

- Cluster：`MC-CLUSTER-0075`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 12 / `parameter_alignment:measurement_accuracy:temperature_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `12 设计输出（含01、02）\设计输出清单二\装配说明V1.0-PT9L.via_xlsx.clean.md:30` measurement_accuracy/temperature_value/temperature: 16.0±2.0℃, 23.0±2.0℃, -0.1℃, -0.2℃, -104℉, -107.6℉, -109.2℉, -40℃; stages: External Test, Stage 01, Stage 04, Stage 11, Stage 12, Stage 18, 历史记录

### PM004. Parameter alignment candidate: operating_environment has multiple numeric values (-003-04, -078-02, -2-56, -20~55, 0 -4, 10~40, 1060-1, 10993-10) across controlled documents.

- Cluster：`MC-CLUSTER-0077`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:operating_environment:range_value:range`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-03 PT9L -2-56 报告-正式.pdf.md:477` operating_environment/range_value/range: -003-04, -078-02, -2-56, -20~55, 0 -4, 10~40, 1060-1, 10993-10; stages: External Test, Stage 01, Stage 03, Stage 04, Stage 11, Stage 12, Stage 18, 历史记录

### PM005. Parameter alignment candidate: measurement_range has multiple numeric values (-003-04, -078-02, -20~55, 0-300, 0 -4, 10~40, 1060-3, 1112-86) across controlled documents.

- Cluster：`MC-CLUSTER-0078`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_range:range_value:range`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-03 PT9L -2-56 报告-正式.pdf.md:477` measurement_range/range_value/range: -003-04, -078-02, -20~55, 0-300, 0 -4, 10~40, 1060-3, 1112-86; stages: External Test, Stage 11, 历史记录

### PM006. Parameter alignment candidate: battery has multiple numeric values (1060-1, 10993-10, 10993-5, 1112-86, 1965-98, 2.6-3, 2002-10, 2020-3) across controlled documents.

- Cluster：`MC-CLUSTER-0079`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 11 / `parameter_alignment:battery:range_value:range`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `11 T1样机\设计验证报告E0 23.11\性能规格部分（一）EN1060-1项.via_xlsx.clean.md:28` battery/range_value/range: 1060-1, 10993-10, 10993-5, 1112-86, 1965-98, 2.6-3, 2002-10, 2020-3; stages: External Test, Stage 04, Stage 06, Stage 11, Stage 12, Stage 18, 历史记录

### PM007. Parameter alignment candidate: battery has multiple numeric values (23.0±2.0℃, -109.2℉, -42.9℃, 0.1°C, 0.2℃, 0.3℃, 0.4°F, 10℃) across controlled documents.

- Cluster：`MC-CLUSTER-0080`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 11 / `parameter_alignment:battery:temperature_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `11 T1样机\PT9L红外体温计检测报告 V1.0.via_lo_html.clean.md:60` battery/temperature_value/temperature: 23.0±2.0℃, -109.2℉, -42.9℃, 0.1°C, 0.2℃, 0.3℃, 0.4°F, 10℃; stages: External Test, Stage 11, Stage 12, Stage 18

### PM008. Parameter alignment candidate: operating_environment has multiple numeric values (-1060h, 0.5s, 060 h, 1 min, 1.4 s, 1.5 s, 1060h, 14.8 h) across controlled documents.

- Cluster：`MC-CLUSTER-0082`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:operating_environment:time_value:time`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-02 PT9L 安规报告-正式.pdf.md:6309` operating_environment/time_value/time: -1060h, 0.5s, 060 h, 1 min, 1.4 s, 1.5 s, 1060h, 14.8 h; stages: External Test, Stage 03, Stage 11, Stage 12, Stage 18, 历史记录

### PM009. Parameter alignment candidate: measurement_accuracy has multiple numeric values (1000-4, 1060-1, 1060-3, 10993-10, 10993-5, 1112-86, 14155-1, 14155-2) across controlled documents.

- Cluster：`MC-CLUSTER-0083`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_accuracy:range_value:range`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-04 PT9L 1965报告-正式.pdf.md:399` measurement_accuracy/range_value/range: 1000-4, 1060-1, 1060-3, 10993-10, 10993-5, 1112-86, 14155-1, 14155-2; stages: External Test, Stage 03, Stage 11, Stage 12, Stage 18, 历史记录

### PM010. Parameter alignment candidate: operating_environment has multiple numeric values (01 V, 1.5V, 15V, 2.7V, 2000 V, 2400 V, 300 V, 42.4 V) across controlled documents.

- Cluster：`MC-CLUSTER-0084`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 07 / `parameter_alignment:operating_environment:voltage_value:voltage`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `07 软件设计方案E0 23.11\质量策划E0 23.11\1 质量策划E0 23.11.via_lo_html.clean.md:64` operating_environment/voltage_value/voltage: 01 V, 1.5V, 15V, 2.7V, 2000 V, 2400 V, 300 V, 42.4 V; stages: External Test, Stage 07, Stage 11, 历史记录

### PM011. Parameter alignment candidate: battery has multiple numeric values (1 s, 10s, 15 s, 2s, 3秒, 30分钟, 48 h, 5 年) across controlled documents.

- Cluster：`MC-CLUSTER-0085`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:battery:time_value:time`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-02 PT9L 安规报告-正式.pdf.md:3852` battery/time_value/time: 1 s, 10s, 15 s, 2s, 3秒, 30分钟, 48 h, 5 年; stages: External Test, Stage 07, Stage 11, Stage 12, Stage 18, 历史记录, 附件1：TZ1准确度的验证报告.via_lo_html.clean.md

### PM012. Parameter alignment candidate: battery has multiple numeric values (0%, 10%, 100 %, 110 %, 20%, 50%, 70%, 75%) across controlled documents.

- Cluster：`MC-CLUSTER-0086`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 18 / `parameter_alignment:battery:percent_value:percent`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `18 设计确认\PT9L usability engineering file.pdf.md:989` battery/percent_value/percent: 0%, 10%, 100 %, 110 %, 20%, 50%, 70%, 75%; stages: External Test, Stage 11, Stage 18, 历史记录

### PM013. Parameter alignment candidate: measurement_range has multiple numeric values (060 h, 1.5 s, 1060 h, 2 h, 24 h, 4 h, 48 h, 5 s) across controlled documents.

- Cluster：`MC-CLUSTER-0087`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 11 / `parameter_alignment:measurement_range:time_value:time`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `11 T1样机\PT3SBT标签审核表（海外更新）.via_xlsx.clean.md:691` measurement_range/time_value/time: 060 h, 1.5 s, 1060 h, 2 h, 24 h, 4 h, 48 h, 5 s; stages: External Test, Stage 11

### PM014. Parameter alignment candidate: operating_environment has multiple numeric values (±0.02℃, ± 0.03 °C, ±0.1°C, ±0.2℃, ± 0.3 ℃, ±0.5 °C, ±1 °C, ±2 °C) across controlled documents.

- Cluster：`MC-CLUSTER-0088`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 12 / `parameter_alignment:operating_environment:tolerance_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `12 设计输出（含01、02）\设计输出清单二\装配说明V1.0-PT9L.via_xlsx.clean.md:28` operating_environment/tolerance_value/temperature: ±0.02℃, ± 0.03 °C, ±0.1°C, ±0.2℃, ± 0.3 ℃, ±0.5 °C, ±1 °C, ±2 °C; stages: External Test, Stage 11, Stage 12, Stage 18, 历史记录, 附件1：TZ1准确度的验证报告.via_lo_html.clean.md

### PM015. Parameter alignment candidate: measurement_range has multiple numeric values (-95%, 10 %, 15 %, 3%, 55%, 70 %, 90 %, 93%) across controlled documents.

- Cluster：`MC-CLUSTER-0090`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_range:percent_value:percent`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-04 PT9L 1965报告-正式.pdf.md:219` measurement_range/percent_value/percent: -95%, 10 %, 15 %, 3%, 55%, 70 %, 90 %, 93%; stages: External Test, Stage 11, 历史记录

### PM016. Parameter alignment candidate: battery has multiple numeric values (±0.03, ±0.05, ±0.1V, ±0.2, ±0.4, ±1mm, ±20, ±50) across controlled documents.

- Cluster：`MC-CLUSTER-0092`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 12 / `parameter_alignment:battery:tolerance_value:tolerance_value`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `12 设计输出（含01、02）\设计输出清单二\图纸\T-Z1-按键-v3.0-20240627.pdf.md:118` battery/tolerance_value/tolerance_value: ±0.03, ±0.05, ±0.1V, ±0.2, ±0.4, ±1mm, ±20, ±50; stages: Stage 04, Stage 11, Stage 12, Stage 18, 历史记录

### PM017. Parameter alignment candidate: measurement_accuracy has multiple numeric values (0.5s, 1分钟, 120 min, 15分钟, 2s, 30分钟, 5 s, 6小时) across controlled documents.

- Cluster：`MC-CLUSTER-0093`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 12 / `parameter_alignment:measurement_accuracy:time_value:time`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `12 设计输出（含01、02）\设计输出清单二\装配说明V1.0-PT9L.via_xlsx.clean.md:29` measurement_accuracy/time_value/time: 0.5s, 1分钟, 120 min, 15分钟, 2s, 30分钟, 5 s, 6小时; stages: External Test, Stage 11, Stage 12, Stage 18, 历史记录

### PM018. Parameter alignment candidate: measurement_accuracy has multiple numeric values (±0, ±0.02, ±0.03, ±0.1V, ±0.2, ±0.4, ±0.5) across controlled documents.

- Cluster：`MC-CLUSTER-0094`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_accuracy:tolerance_value:tolerance_value`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-03 PT9L -2-56 报告-正式.pdf.md:354` measurement_accuracy/tolerance_value/tolerance_value: ±0, ±0.02, ±0.03, ±0.1V, ±0.2, ±0.4, ±0.5; stages: External Test, Stage 11, Stage 12, Stage 18, 历史记录

### PM019. Parameter alignment candidate: operating_environment has multiple numeric values (±0.05, ±0.2, ±0.4, ±0.5, ± 1, ±2) across controlled documents.

- Cluster：`MC-CLUSTER-0095`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:operating_environment:tolerance_value:tolerance_value`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-04 PT9L 1965报告-正式.pdf.md:309` operating_environment/tolerance_value/tolerance_value: ±0.05, ±0.2, ±0.4, ±0.5, ± 1, ±2; stages: External Test, Stage 11, Stage 18, 历史记录

### PM020. Parameter alignment candidate: measurement_accuracy has multiple numeric values (±0.02℃, ± 0.1 °C, ±0.2℃, ± 0.3 ℃, ±2.0℃) across controlled documents.

- Cluster：`MC-CLUSTER-0096`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 11 / `parameter_alignment:measurement_accuracy:tolerance_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `11 T1样机\PT9L红外体温计产品检验标准 V1.0.doc.md:200` measurement_accuracy/tolerance_value/temperature: ±0.02℃, ± 0.1 °C, ±0.2℃, ± 0.3 ℃, ±2.0℃; stages: External Test, Stage 01, Stage 04, Stage 11, Stage 12, Stage 18, 历史记录

### PM021. Parameter alignment candidate: measurement_range has multiple numeric values (±0, ±0.2, ±0.4, ±0.5, ±1) across controlled documents.

- Cluster：`MC-CLUSTER-0097`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_range:tolerance_value:tolerance_value`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-03 PT9L -2-56 报告-正式.pdf.md:354` measurement_range/tolerance_value/tolerance_value: ±0, ±0.2, ±0.4, ±0.5, ±1; stages: External Test, Stage 11, 历史记录

### PM022. Parameter alignment candidate: measurement_accuracy has multiple numeric values (0.03 v, 0.1V, 01 V, 2.7V, 3.3V) across controlled documents.

- Cluster：`MC-CLUSTER-0098`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 12 / `parameter_alignment:measurement_accuracy:voltage_value:voltage`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `12 设计输出（含01、02）\设计输出清单二\图纸\开关帽.pdf.md:151` measurement_accuracy/voltage_value/voltage: 0.03 v, 0.1V, 01 V, 2.7V, 3.3V; stages: External Test, Stage 07, Stage 11, Stage 12, Stage 18, 历史记录

### PM023. Parameter alignment candidate: measurement_range has multiple numeric values (± 0.1 °C, ± 0.2 ℃, ± 0.3 ℃) across controlled documents.

- Cluster：`MC-CLUSTER-0100`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_range:tolerance_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-04 PT9L 1965报告-正式.pdf.md:473` measurement_range/tolerance_value/temperature: ± 0.1 °C, ± 0.2 ℃, ± 0.3 ℃; stages: External Test, Stage 11, 历史记录

### PM024. Parameter alignment candidate: operating_environment has multiple numeric values (±10%, ±20%, ±3%) across controlled documents.

- Cluster：`MC-CLUSTER-0101`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 18 / `parameter_alignment:operating_environment:tolerance_value:percent`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `18 设计确认\010_Attachment 8. Clinical study report修改20200610.via_lo_html.clean.md:68` operating_environment/tolerance_value/percent: ±10%, ±20%, ±3%; stages: External Test, Stage 11, Stage 12, Stage 18, 历史记录, 附件1：TZ1准确度的验证报告.via_lo_html.clean.md

### PM025. Parameter alignment candidate: battery has multiple numeric values (±10 %, ±20%) across controlled documents.

- Cluster：`MC-CLUSTER-0102`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:battery:tolerance_value:percent`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
...
### PM026. Parameter alignment candidate: measurement_range has multiple numeric values (±10 %, ±3%) across controlled documents.

- Cluster：`MC-CLUSTER-0103`
- 严重度：`P2`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：External Test / `parameter_alignment:measurement_range:tolerance_value:percent`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `实验报告\最终报告\2025.02.12 ETX202303-07-078-02 PT9L 安规报告-正式.pdf.md:548` measurement_range/tolerance_value/percent: ±10 %, ±3%; stages: External Test, Stage 11

### PM027. Parameter alignment candidate: battery has multiple numeric values (-3V, 0.1 V, 01 V, 02 V, 04 V, 1.5V, 100 V, 12 V) across controlled documents.

- Cluster：`MC-CLUSTER-0076`
- 严重度：`P3`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 06 / `parameter_alignment:battery:voltage_value:voltage`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `06 硬件设计方案E0 23.11\09 硬件文档E0-PT9L-模板\09 硬件文档E0\E0版硬件方案阶段\硬件设计方案评审检查表.via_xlsx.clean.md:16` battery/voltage_value/voltage: -3V, 0.1 V, 01 V, 02 V, 04 V, 1.5V, 100 V, 12 V; stages: External Test, PT9L-DHF清单-常规.via_xlsx.clean.md, Stage 02, Stage 03, Stage 06, Stage 07, Stage 11, Stage 12

### PM028. Parameter alignment candidate: operating_environment has multiple numeric values (50±20%, 55±10%, -70%, -85%, -95%, 0%, 10%, 100 %) across controlled documents.

- Cluster：`MC-CLUSTER-0081`
- 严重度：`P3`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 18 / `parameter_alignment:operating_environment:percent_value:percent`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `18 设计确认\废案与备份\附件1：TZ1准确度的验证报告.via_lo_html.clean.md:36` operating_environment/percent_value/percent: 50±20%, 55±10%, -70%, -85%, -95%, 0%, 10%, 100 %; stages: External Test, K040100014357620814-图纸-PT9L-F01+V1.0.pdf.md, Stage 01, Stage 04, Stage 11, Stage 12, Stage 18, 历史记录

### PM029. Parameter alignment candidate: measurement_accuracy has multiple numeric values (-95%, 100%, 15%, 20%, 50%, 70%, 75%, 85%) across controlled documents.

- Cluster：`MC-CLUSTER-0089`
- 严重度：`P3`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：历史记录 / `parameter_alignment:measurement_accuracy:percent_value:percent`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `历史记录\PT9LDHF-草稿\设计确认\7.临床评价报告.via_lo_html.clean.md:68` measurement_accuracy/percent_value/percent: -95%, 100%, 15%, 20%, 50%, 70%, 75%, 85%; stages: External Test, Stage 11, Stage 12, Stage 18, 历史记录

### PM030. Parameter alignment candidate: measurement_range has multiple numeric values (01 V, 1.5V, 12 V, 12.4 V, 15.1 V, 2.7V, 24.8 V, 3V) across controlled documents.

- Cluster：`MC-CLUSTER-0091`
- 严重度：`P3`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 11 / `parameter_alignment:measurement_range:voltage_value:voltage`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `11 T1样机\软件文件底稿\软件需求E0 23.11\2-1 软件需求规格E0 23.11.via_lo_html.clean.md:124` measurement_range/voltage_value/voltage: 01 V, 1.5V, 12 V, 12.4 V, 15.1 V, 2.7V, 24.8 V, 3V; stages: External Test, Stage 11, 历史记录

### PM031. Parameter alignment candidate: battery has multiple numeric values (±0.1°C, ±0.2℃, ±0.3℃, ±2.0℃) across controlled documents.

- Cluster：`MC-CLUSTER-0099`
- 严重度：`P3`
- Agent：Hardware Agent
- 类型：`parameter_alignment_candidate`
- 阶段/术语：Stage 18 / `parameter_alignment:battery:tolerance_value:temperature`
- 涉及候选行数：1
- 未决原因：机械候选最终裁决 Agent 调用失败，保留人工复核：APIConnectionError: Connection error.
- 代表证据：
  - `18 设计确认\废案与备份\7.临床评价报告.via_lo_html.clean.md:71` battery/tolerance_value/temperature: ±0.1°C, ±0.2℃, ±0.3℃, ±2.0℃; stages: External Test, Stage 11, Stage 18
...
### S004. 整机寿命测试报告存在三个版本文件（F0084、F0085、F0095），文件编号相同但来源不同，版本控制状态不明确。
...
### S005. 结构类FMEA中电池功能描述为"给血压计供电"，与PT9L红外体温计品类不符。
...
### S006. 多份样机评审检查表中所有检查项结果栏均为空白（无通过/不通过/不适用勾选），评审完成状态无法确认。
...
### S007. 主范围风险评估报告（F0018）正文两处均出现旧型号PT3SBT，预期用途描述与PT9L项目不符。
...
### S008. 03风险分析文件夹中存在两套结构相同的风险管理文件（F0017-F0020无PT9L前缀 vs F0021-F0023有PT9L前缀），受控版本不明确。
...
### S009. 正式实验报告（F0458，PT9L -2-56测试报告）中引用的风险管理报告文件号为PT9C-FXGB01，与PT9L项目文件号体系不一致。
...
### S010. PT9L T1样机文件夹中存在PT3SBT标签审核表，且检查记录引用PT2L标签文件，旧型号残留污染主范围文件，法规证据归属不清。
...
### S011. 风险评估报告和FMEA均引用IEC 60601-1作为适用标准，但红外体温计通常为非电气设备，IEC 60601-1适用性需正式说明。
...
### S012. 设计验证报告文件名含'EN1060-1项'，EN 1060-1为血压计标准，疑为旧品类模板残留，影响验证报告与IFT法规要求的对应关系。
...
### S013. 风险管理报告（F0020）文件名标注IFT-FXGB01 V2.0，但正文文件号字段显示PT9L-FXGB01 V1.0，文件号与版本号双重矛盾。
...
### S014. F0022（PT9L版风险评价报告）文件名标注V2.0，但正文DN字段显示PT9L-FXPJ01 V1.0，版本号不一致。
...
### S015. F0023（PT9L版风险控制验证报告）文件名标注V2.0，但正文DN字段显示PT9L-FXYS01 V1.0，版本号不一致。
...
### S016. 结构类DFMEA（F0074）严重度评分表中明确写入"影响血压计的安全运行"，品类描述与PT9L红外体温计不符，构成旧品类残留。
...
### S017. PT9L机芯类组件清单中PCB图号引用PT9C旧型号文件编号，存在设计输出与当前型号不一致的法规风险
...
### S018. PT3SBT/PT2L旧型号标签审核表存放于PT9L T1样机文件夹，文件内容与PT9L无关，构成旧型号资料残留
...
### S019. 设计确认阶段验证报告封皮中"设计验证合格"结论项为"☐"（未勾选），设计确认闭环结论缺失。
...
### S020. 散热器压块图纸（FDIR-V14）在PT9L总装图BOM中以PT3-M01入账，图号前缀为PT3而非PT9L，与PT9L项目图号命名规范不符，存在物料可追溯性风险。
...
### S021. F0162包装BOM（受控文件PT9L-PB0M1 V1.0）中仅列出彩包装和纸托2项，而F0161包装BOM列出4项（含说明书PT9L-SMSPO1和保护膜PT9L-P10），两份BOM物料条目数量不一致，存在漏料风险。
...
### S022. 设计验证计划通用要求部分含"次血压测量"字样，与PT9L红外体温计品类不符，构成品类污染风险。
...
### S023. 设计输入中EN ISO 80601-2-56:2017+A1:2018被标记为'否（不适用）'，但说明书声明符合该标准，存在法规适用性矛盾。
...
### S024. DHF中存在两套内容高度相似的风险管理文件（F0017/F0018/F0019与F0021/F0022/F0023），版本号和日期相同，文件归属和主控版本不明确。
...
### S025. F0242（设计确认文件）文件名含PT9C，但其中引用的风险管理报告为PT9L-FXGB01 V1.0，文件归属与引用内容存在型号混用。
...
### S026. 设计验证计划（F0121）中YY/T 0316-2016、EN ISO 14971:2019等风险相关标准均被标注为"否"（不适用），与项目实际执行风险管理活动相矛盾。
...
### S027. 子程序测试记录中记录的软件版本号PT-HT2762-V0029-1.0.0与软件需求规格书、设计方案的版本对应关系在证据目录中缺失
...
### S028. 证据目录中未见独立的软件确认报告，软件质量策划声称包含确认计划但确认执行结果文件无法从现有证据确认存在
...
### S029. T1样机评审检查表中软件功能检查条目（程序版本一致性、通信协议版本、APP/云端版本）全部为空白，T1阶段软件评审结论无法确认
...
### S030. DHF主范围文件中混入PT3SBT型号的标签审核表，与PT9L项目无关，构成型号混淆风险。
...
### S031. 通用要求验证报告中外形/重量和包装要求两项验证结果全部标注为"--"，未实际执行验证。
...
### S032. 设计输入汇总表文件名含日期"20211118"，而设计输入封皮版本为"E0 23.11"，日期不一致，存在版本对应关系不明的风险。
...
### S033. 手板样机设计评审记录中设计验证报告评估栏四个选项均为"□"，未勾选任何验证状态，评审结论不完整。
...
### S034. 外观图（PT9L-F01 V1.0）仅标注上盖白色，未标注Cool Gray 8U装饰色规格，与彩包装图（DMR-082）描述的橙/白双色外观存在潜在不一致，外观验收标准不完整。
...
### S035. 低电压第一阶段触发阈值在构建编码文件与软件测试记录之间存在描述差异（2.5~2.7V vs 2.65V），需确认实际固件实现值。
...
### S036. 主范围结构设计方案评审检查表（F0035）文件名及标题均标注"血压计"，与PT9L红外体温计品类不符。
...
### S037. 设计验证报告封皮的'验证合格'与'验证不合格'复选框均未勾选，正式验证结论缺失，法规闭环不完整。
...
### S038. 所有生物相容性测试报告均以PT5为受试样品，证据目录中未见PT9L材料等同性桥接文件，生物相容性法规证据闭环存在缺口。
...
### S039. 风险管理计划（F0017/F0021）版本均为V1.0（2024.02.02），而风险评价报告和风险控制验证报告均已更新至V2.0（2024.05.30），计划文件未同步更新。
...
### S040. 可追溯性分析报告（F0252）在验证与确认阶段引用风险评价报告PT9L-FXPJ01 V1.0，但文件目录中已存在V2.0版本，引用版本未更新。
...
### S041. PT9L软件设计方案文件夹下存放BP3L旧型号软件图样，该文件未被排除出主范围，构成法规验证闭环污染风险
...
### S042. 软件图样评审计划及评审记录所指向的评审对象疑为BP3L图样而非PT9L图样，评审有效性存疑
...
### S043. 结构类FMEA文件中产品型号KD-5915L与PT9L混用，同一文件内出现两个不同型号标识
...
### S044. 软件需求规格评审计划中引用的附表文件编号PT9L-RXPB01与软件方案评审计划引用的PT9L-RFPB01格式不一致，存在文件编号规则混乱风险
...
### S045. 性能规格验证报告（F0112）Sheet名为"组件验证报告"，但文件标题和内容均为"性能规格部分"，存在Sheet命名与内容不符的一致性风险。
...
### P002. 电池使用寿命测试报告中电池容量标注为1000mAh，与包装BOM中1200mAh不一致，测试代表性存疑。

- Finding ID：`LLM-0022`
- 严重度：`P1`
- Agent：Hardware Agent
- 类型：`battery_parameter_inconsistency`
- 未决原因：Challenge Agent 的原始裁决理由过短或疑似被截断，当前不能稳健支持 `keep` 裁决，已保守转为 needs_more_context；原始理由：证据直接支持结论：line 542和543明确写明
- 现有证据：
...
### P003. 主范围风险控制验证报告（F0019）正文中出现旧型号PT3SBT，而非PT9L，导致预期用途描述与本项目不符。
...
### P004. 主范围DFMEA文件（F0074）多处内容明确描述血压计功能（电池给血压计供电、包装血压计等），与PT9L红外体温计品类不符。

- Finding ID：`LLM-0006`
- 严重度：`P1`
- Agent：Market & Category Scout Agent
- 类型：`legacy_category_in_primary_scope`
- 未决原因：Challenge Agent 的原始裁决理由过短或疑似被截断，当前不能稳健支持 `keep` 裁决，已保守转为 needs_more_context；原始理由：证据直接且充分：结构类FMEA（F0074 line 22）明确写
- 现有证据：
  - `11 T1样机\05 DFMEA E0 23.11\结构类FMEA  E0(1)(1).via_xlsx.clean.md:17` \| \| 电池 \| 给血压计供电 \| 电池无法装入电池仓 \| 用户无法测量 \| 8 \| B \| 电池的尺寸超出设计要求 \| 3 \| \| 实际装配 \| 5 \| 120 \| 尺寸检验 \| 8 \| 3 \| 2 \| 48 \| 检查尺寸检测报告 \| \| \| □ \|
...
### P005. 主范围设计输出清单二（F0235、F0236）中大量条目引用PT3SBT文件编号，与PT9L项目不符。

- Finding ID：`LLM-0008`
- 严重度：`P1`
- Agent：Market & Category Scout Agent
- 类型：`legacy_model_in_primary_scope`
- 未决原因：Challenge Agent 的原始裁决理由过短或疑似被截断，当前不能稳健支持 `keep` 裁决，已保守转为 needs_more_context；原始理由：证据直接支持结论：设计输入汇总表（line 134）明确勾选
- 现有证据：
...
### P006. 结构类FMEA中将产品描述为'血压计'，与PT9L红外体温计品类不符，构成法规/验证闭环风险。
...
### P007. DHF中存在两套平行风险文件：IFT前缀（F0017-F0020）与PT9L前缀（F0021-F0024），均为同名文件，未明确哪套为正式受控版本。
...
### P008. 生物相容性报告（皮肤刺激、皮肤致敏、细胞毒性）均以PT5为受试样品，未见PT9L专属生物相容性评价证据。
...
### P009. 散热器金属套图纸（75CEE0-Model.pdf）为外购供应商图号（GXT-001/天津小泉），尚未完成内部标准图号映射，制造转移资料中图号体系不统一。
...
### P010. T1样机评审检查表中含"电池不能和袖带相邻存放"检查项，袖带为血压计专用部件，与IFT品类不符。
...
### P011. 主范围软件设计方案文件夹中存在BP3L软件图样（F0052），与PT9L项目型号不符。

- Finding ID：`LLM-0007`
- 严重度：`P2`
- Agent：Market & Category Scout Agent
- 类型：`legacy_model_in_primary_scope`
- 未决原因：Challenge Agent 的原始裁决理由过短或疑似被截断，当前不能稳健支持 `revise` 裁决，已保守转为 needs_more_context；原始理由：核心问题成立：PT9L DHF中存在文件名为
- 现有证据：
  - `07 软件设计方案E0 23.11\软件图样E0 23.11\BP3L 软件图样E0 23.11.via_lo_html.clean.md:2` # BP3L 软件图样E0 23.11
...
### P012. 主范围标签审核表（F0082）文件名为PT3SBT，内容引用PT2L型号及文件编号，与PT9L项目不符。

- Finding ID：`LLM-0009`
- 严重度：`P2`
- Agent：Market & Category Scout Agent
- 类型：`legacy_model_in_primary_scope`
- 未决原因：Challenge Agent 的原始裁决理由过短或疑似被截断，当前不能稳健支持 `keep` 裁决，已保守转为 needs_more_context；原始理由：证据支持结论：所有生物相容性报告文件名均含
- 现有证据：
  - `11 T1样机\PT3SBT标签审核表（海外更新）.via_xlsx.clean.md:2` # PT3SBT标签审核表（海外更新）
  - `11 T1样机\PT3SBT标签审核表（海外更新）.via_xlsx.clean.md:8` PT2L(美亚)标签检查报告
  - `11 T1样机\PT3SBT标签审核表（海外更新）.via_xlsx.clean.md:12` 产品型号： PT2L 项目编号： 2020-DEV01 产品/项目名称：红外体温计

### P013. 主范围设计输入汇总表（F0031）与历史草稿（F0319）在EN ISO 80601-2-56和MDR 2017/745适用性勾选上存在矛盾，正式版本选项需复核。
...
### P014. 说明书声明符合ISO 80601-2-56但'except of clause 5.2.2'，该豁免条款未在设计输入或风险文件中找到正式说明。
...
### P015. 设计输入和设计验证报告均未勾选CE认证，但说明书和风险文件多处引用EN标准，市场范围与标准引用存在不一致。
...
### P016. 用户现场测试计划（F0260）文件名含PT9C，但内容包含风险评估条款，未见PT9L对应版本的用户测试计划证据。
...
### P017. EMC型式试验报告（F0456）明确要求免疫通过/失败准则应纳入风险管理文件，但现有证据目录中未见风险管理文件包含EMC免疫准则的对应条款。
...
### P018. 软件质量策划评审检查表、软件需求规格书评审检查表、软件设计方案评审检查表的评审结论栏全部为空白复选框，评审是否通过无法确认
...
### P019. T1阶段设计验证报告封皮中三个子报告勾选框均为"□"（未勾选），无法确认T1验证已正式完成并通过。
...
### P020. 设计验证计划封皮（F0118）中子计划勾选框状态无法从证据中确认，存在验证计划正式批准状态不明的风险。
...
### P021. 产品检验标准中允许对不合格温度计复测两次且两次合格即判合格，该规则是否已在验证计划判定准则中明确引用存疑。
...
## 9. Challenge 理由质量保守降级

- `LLM-0006` 原裁决 `keep` 被降级为 `needs_more_context`。
  原始理由：证据直接且充分：结构类FMEA（F0074 line 22）明确写
- `LLM-0007` 原裁决 `revise` 被降级为 `needs_more_context`。
  原始理由：核心问题成立：PT9L DHF中存在文件名为
- `LLM-0008` 原裁决 `keep` 被降级为 `needs_more_context`。
  原始理由：证据直接支持结论：设计输入汇总表（line 134）明确勾选
- `LLM-0009` 原裁决 `keep` 被降级为 `needs_more_context`。
  原始理由：证据支持结论：所有生物相容性报告文件名均含
- `LLM-0022` 原裁决 `keep` 被降级为 `needs_more_context`。
  原始理由：证据直接支持结论：line 542和543明确写明

## 10. 关联输出文件

- `PT9L_full_audit_report_langgraph.md`：全量审查报告。
- `PT9L_mechanical_cluster_decision_table.md`：机械候选最终裁决总表，按 cluster 去重。
- `PT9L_mechanical_review_true_errors.md`：机械确认错误候选行展开版。
- `PT9L_mechanical_review_dismissed.md`：机械候选驳回/非错误清单。
- `second_round_tasks.md`：仍需补证的语义疑点任务。